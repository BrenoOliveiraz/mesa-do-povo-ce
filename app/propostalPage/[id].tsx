...
export default function ItemProposta() {
  ...
  useEffect(() => {
    const fetchProposta = async () => {
      try {
        ...
        //TODO: PEGAR ESSE CPF DINAMICAMENTE DEPOIS DO USUARIO LOGADO
  ...
  console.log(proposta.produtos)

  return (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      {/* Header */}
      <View style={styles.header}>
        <Text style={styles.headerTitle}>{proposta.nomeProponente}</Text>
        <Text style={styles.headerSubtitle}>CNPJ: {proposta.cnpjProponente}</Text>
        <Text style={styles.headerSubtitle}>TPAF: {proposta.numTpaf}</Text>
      </View>
